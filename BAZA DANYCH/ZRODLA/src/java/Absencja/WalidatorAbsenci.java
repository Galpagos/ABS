...
public class WalidatorAbsenci {
	...
	public boolean czyPrzekraczaLimity(AbsencjaDTO pmAbsencja) {
		...
		if (lvWynik && pmAbsencja.getRodzaj() != SLRodzajeAbsencji.ciaza
			...
			int reply = JOptionPane.showConfirmDialog(null,
					"Przekroczono limit " + lvLimit + " dni dla absencji " + pmAbsencja.getRodzaj().getNazwa()
							+ ".\n Uwzględniając wprowadzoną absencje liczba dni nieobecności wynosi "
	...
	public boolean czyDniL4Ciagiem(AbsencjaDTO pmAbsencja) {
		...
		List<AbsencjaDTO> lvOgraniczona = lvLista.stream()//
				.filter(lvAbs -> Arrays
						.asList(SLRodzajeAbsencji.L_4, SLRodzajeAbsencji.szpital, SLRodzajeAbsencji.ciaza)
		...
		for (int i = lvOgraniczona.size() - 1; i >= 0; i--) {
			...
			if (licznik >= lvDni) {
				JOptionPane.showMessageDialog(null,
						"Dla pracownika należy wykonać nowe badania lekarskie ze względu na długą niedyspozycyjność");
				return true;
			}
